    ...
    public void actionPerformed(ActionEvent ae){
        try{        
            if(ae.getSource()==b1){
                ...
                String pinCode = new String(pf2.getPassword());

                // Query
                String q  = "select * from login where card_number = '" + cardNumber + "' and pin_code = '"+ pinCode +"'";

                ResultSet rs = c1.s.executeQuery(q);
                if(rs.next()){
                    setVisible(false);
                    new MainScreen(pinCode).setVisible(true);
                } else{
